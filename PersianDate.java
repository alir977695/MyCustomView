package com.example.ali.khatm.PersianDate;
...
/**
 * Created by ALI-PC on 11/9/2017.
 */
...
public class PersianDate {
    ...
    public void addOffset(int add){
        this.offset += add;
        int i = this.date.getDate();
        this.date.setDate(this.date.getDate() + add);
    }
    public int getOffset() {
        return offset;
    }
    public void setOffset(String date){
        int year = Integer.parseInt(date.substring(0,4));
        int month = Integer.parseInt(date.substring(5,7));
        int day = Integer.parseInt(date.substring(8,10));
        setOffset(0);
        int jYear = jalali.getYear(this.date);
        while (jYear != year){
            if(jYear > year){
                addOffset(-365);
            }
            ...
            jYear = jalali.getYear(this.date);
        }
        int jMonth = jalali.getMonth(this.date);
        while (jMonth != month){
            if(jMonth > month){
                addOffset(-28);
            }
            ...
            jMonth = jalali.getMonth(this.date);
        }
        int jDay = jalali.getDay(this.date);
        while (jDay != day){
            if(jDay > day){
                addOffset(-1);
            }
            ...
            jDay = jalali.getDay(this.date);
        }
    }
    public int getDay(){
        return this.jalali.getDay(date);
    }
    public int getMonth(){
        return this.jalali.getMonth(date);
    }
    public int getYear(){
        return this.jalali.getYear(date);
    }
    public String getMonthString(){
        switch (getMonth()){
            ...
            case 10:return "دي";
            case 11:return "بهمن";
            default:return "اسفند";
        }
    }
    public int getDayOfWeek(){
        return (JalaliCalendar.dayOfWeek(new JalaliCalendar.YearMonthDate(date.getYear(),date.getMonth(),date.getDate())) + 3)%7;
    }
    ...
    public int getFirstDayOfWeekOffset(){
        return -this.getDayOfWeek();
    }

    public String getDate(){
        return jalali.getJalaliDate(date);
    }
    public String getDateNumber(){
        return jalali.getJalaliDateNumber(date);
    }
    public static String getCurrentDate() {
        return (new JalaliCalendar()).getJalaliDate(new Date());
    }
    public static String getCurrentDateNumber(){
        return (new JalaliCalendar()).getJalaliDateNumber(new Date());
    }
    ...
    public int getMonthLength() {
        ...
        else if ((this.getYear() - 1395)%4 == 0)
            return 30;
        return 29;
    }

    public static class JalaliCalendar extends Calendar {
        ...
        public String getJalaliDateNumber(Date gDate) {
            ...
            String jalali = jDate.year + "/" + String.format(aLocale,"%02d",jDate.month + 1) + "/" + String.format(aLocale,"%02d",jDate.date);

            return jalali;
        }

        public int getYear(Date gDate){
            Calendar gCal = Calendar.getInstance();
        ...
        public int getMonth(Date gDate){
            Calendar gCal = Calendar.getInstance();
            ...
            YearMonthDate jDate = gregorianToJalali(gMyDate);
            return jDate.month + 1;
        }
        public int getDay(Date gDate){
            Calendar gCal = Calendar.getInstance();
        ...
        public String getJalaliDate(Date gDate){
            ...
            switch (jDate.month){
                case 0:return String.format(aLocale,"%02d",jDate.date)+ "/" + "فروردين" + "/" + jDate.year ;
                case 1:return String.format(aLocale,"%02d",jDate.date)+ "/" + "ارديبهشت" + "/" + jDate.year ;
                case 2:return String.format(aLocale,"%02d",jDate.date)+ "/" + "خرداد" + "/" + jDate.year ;
                case 3:return String.format(aLocale,"%02d",jDate.date)+ "/" + "تير" + "/" + jDate.year ;
                case 4:return String.format(aLocale,"%02d",jDate.date)+ "/" + "مرداد" + "/" + jDate.year ;
                case 5:return String.format(aLocale,"%02d",jDate.date)+ "/" + "شهريور" + "/" + jDate.year ;
                case 6:return String.format(aLocale,"%02d",jDate.date)+ "/" + "مهر" + "/" + jDate.year ;
                case 7:return String.format(aLocale,"%02d",jDate.date)+ "/" + "آبان" + "/" + jDate.year ;
                case 8:return String.format(aLocale,"%02d",jDate.date)+ "/" + "آذر" + "/" + jDate.year ;
                case 9:return String.format(aLocale,"%02d",jDate.date)+ "/" + "دي" + "/" + jDate.year ;
                case 10:return String.format(aLocale,"%02d",jDate.date)+ "/" + "بهمن" + "/" + jDate.year ;
                default:return String.format(aLocale,"%02d",jDate.date)+ "/" + "اسفند" + "/" + jDate.year ;
            }
        }
